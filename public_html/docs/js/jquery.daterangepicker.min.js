...
(function(factory) {
    ...
    $.fn.dateRangePicker = function(opt) {
        ...
        function init_datepicker() {
			...
			box.find('.time1 input[type=time]').on('change touchmove', function(e) {
				var valueArr = $(this).val().split(':').map((item) => {
					return +item;
				});

                var 
                    hour = valueArr[0] != '' ? valueArr[0] : undefined,
                    min = valueArr[1] != '' ? valueArr[1] : undefined;
                setTime('time1', hour, min);
            });
			...
			box.find('.time2 input[type=time]').on('change touchmove', function(e) {
				var valueArr = $(this).val().split(':').map((item) => {
					return +item;
				});
            ...
            if (!opt.inline) {
                ...
                if ($(opt.container).css('position') == 'relative') {
                    ...
                    if (offset.left < 460) //left to right
                    {
						...
						if($(self).width() > 240) insleft = offset.left + $(self).width() - box.width() - 21;
						else insleft = offset.left + $(self).width() - box.width() + 47;
        ...
        function renderTime(name, date) {
			...
			box.find('.' + name + ' input[type=time].vremya-range').val(moment(date).hours()+':'+moment(date).minutes());
        ...
        function setTime(name, hour, minute) {
			...
			//box.find('.' + name + ' .vremya-range').val(hour+':'+minute);
            switch (name) {
                case 'time1':
                    if (opt.start) {
        ...
        function getTimeHTML() {
            return '<div>' +
                ...
                '</div>' +
                '<div class="vremya">' +
                '<label><input type="time"  min="10:00" max="21:00" step="600" class="vremya-range" name="vremya"></label>' +
                '</div>';
        }
        ...
        function createDom() {
            ...
            html += '<div class="month-wrapper">' +
                ...
                '       <thead>' +
                '           <tr class="caption">' +
				'               <th colspan="7"><div class="month-flex"><div class="month-name">' +
                '               </div>' +
                '               <div class="th_arrow">' +
                '                   <span class="prev">' + arrowPrev + '</span>' +
				(opt.singleDate || !opt.stickyMonths ? '<span class="next">' + arrowNext + '</span>' : '') +
                '               </div>' +
                '               <div class="th_space"></div><div class="time">' +
                '					<div class="time1"></div>' +
                '               </div></div></th>' +
                '           </tr>' +
